...
import { isQuiet, formatIsJson, formatIsCsv, formatIsParsingCompatible } from './options';
...
function isINamedTable(object: any): object is INamedTable {
    return (
        ...
        object.content.every(
            (item: any) =>
                isINamedTable(item) || (item instanceof Array && item.every((itemComponent) => typeof itemComponent === 'string'))
        )
    );
}
